use crate::ai::general_node::GeneralNodeState;
...
impl<'a> GeneralHeuristic<'a, CombinedEnc<'a>> for NaiveHeuristic {
    type GeneralEnc = ();
...
impl<'a> GeneralChildGen<'a> {
    ...
    fn propose_assignment<C, H: GeneralHeuristic<'a, C>>(
        ...
        let acquire: Vec<usize> = self.nodes[cur_idx].tech_indices.iter().cloned().collect();
    ...
    for GeneralChildGen<'a>
{
    ...
    fn new(state: &GeneralNodeState<'a, C, H>, rng: &mut impl Rng, args: Self::Args) -> Self {
        ...
        let mut scored: Vec<(usize, f32)> = techline
            .techs
            .iter()
            .enumerate()
            .filter_map(|(i, t)| {
