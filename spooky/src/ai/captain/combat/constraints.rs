...
#[derive(Debug, Clone)]
pub struct SatVariables {
    // Passive variables - only for pieces that could attack
    pub passive: HashMap<Loc, Bool>, // whether the piece is engaged in combat

    // Attack variables - only for non-passive attackers
    pub attack_hex: HashMap<Loc, Z3Loc>, // which hex the attacker moved to before attacking
    pub attack_time: HashMap<Loc, Z3Time>, // on which time the attacker attacked

    // Attack relationship variables
    pub attacked: HashMap<(Loc, Loc), Bool>, // whether the attacker attacked defender
    pub num_attacks: HashMap<(Loc, Loc), Z3HP>, // number of times attacker attacked defender

    // Removal variables
    // pub removed: HashMap<Loc, Bool>, // whether defender got removed
    pub killed: HashMap<Loc, Bool>,     // whether defender got killed
    pub unsummoned: HashMap<Loc, Bool>, // whether defender got unsummoned
    pub removal_time: HashMap<Loc, Z3Time>, // when defender got removed (killed or unsummoned)
    ...
    pub move_hex: HashMap<Loc, Z3Loc>, // what hex friendly unit is moving to
    pub move_time: HashMap<Loc, Z3Time>, // when the friendly unit moves

    // Blink variables
    pub blink: HashMap<Loc, Bool>, // whether an attacker with blink keyword is blinking
}

impl SatVariables {
    pub fn new(_ctx: &Context, graph: &CombatGraph, board: &Board) -> Self {
        ...
        for attacker in &graph.attackers {
            // Passive variable
            vars.passive
                .insert(*attacker, Bool::new_const(format!("passive_{}", attacker)));

            // Attack variables (will be constrained based on passive status)
            vars.attack_hex.insert(
                *attacker,
                Z3Loc::new_const(format!("attack_hex_{}", attacker), BV_LOC_SIZE),
            );
            vars.attack_time.insert(
                *attacker,
                Z3Time::new_const(format!("attack_time_{}", attacker), BV_TIME_SIZE),
            );
            ...
            if piece.unit.stats().blink {
                vars.blink
                    .insert(*attacker, Bool::new_const(format!("blink_{}", attacker)));
            }
        }
        ...
        for pair in &graph.triples {
            ...
            vars.attacked
                .insert(key, Bool::new_const(format!("attacked_{}{}", key.0, key.1)));
            vars.num_attacks.insert(
                key,
                Z3HP::new_const(format!("num_attacks_{}{}", key.0, key.1), BV_HP_SIZE),
            );
        }
        ...
        for defender in &graph.defenders {
            vars.killed
                .insert(*defender, Bool::new_const(format!("killed_{}", defender)));
            vars.unsummoned.insert(
                *defender,
                Bool::new_const(format!("unsummoned_{}", defender)),
            );
            vars.removal_time.insert(
                *defender,
                Z3Time::new_const(format!("removal_time_{}", defender), BV_TIME_SIZE),
            );
        }
    ...
    pub fn attack_move<'a>(&'a self, attacker: &Loc) -> Z3Move<'a> {
        Z3Move {
            from: *attacker,
    ...
    pub fn friend_move<'a>(&'a self, loc: &Loc) -> Z3Move<'a> {
        Z3Move {
            from: *loc,
    ...
    pub fn add_friendly_movement_variable(&mut self, _ctx: &Context, loc: Loc, blink: bool) {
        ...
        self.move_hex.insert(
            loc,
            Z3Loc::new_const(format!("move_hex_{}", loc), BV_LOC_SIZE),
        );
        self.move_time.insert(
            loc,
            Z3Time::new_const(format!("move_time_{}", loc), BV_TIME_SIZE),
        );

        if blink && !self.blink.contains_key(&loc) {
            self.blink
                .insert(loc, Bool::new_const(format!("blink_{}", loc)));
        }
    }
}
...
pub struct ConstraintManager {
    pub ctx: Context,
    pub solver: Solver,
    pub variables: SatVariables,
    pub graph: CombatGraph,
}

impl ConstraintManager {
    pub fn new(ctx: &Context, graph: CombatGraph, board: &Board) -> Self {
    ...
    pub fn blink_constraint<'a>(&self, move1: &Z3Move<'a>, move2: &Z3Move<'a>) -> Bool {
        let Z3Move {
            from: loc1,
            to: dest1,
            time: time1,
        ...
        let same_hex = (*dest1).clone().eq((*dest2).clone());
        ...
        let blink_resolves = match (
            ...
            (Some(blink1), Some(blink2)) => {
                let first = (*time1).clone().bvult((*time2).clone());
                let second = (*time2).clone().bvult((*time1).clone());

                Bool::or(&[
                    &Bool::and(&[&first, blink1]),
                    &Bool::and(&[&second, blink2]),
                ])
            }
            (Some(blink1), None) => {
                let first = (*time1).clone().bvult((*time2).clone());
        ...
        for (attacker, defenders) in &self.graph.attacker_to_defenders_map {
            ...
            for other_attacker in &self.graph.attackers {
                if other_attacker != attacker {
                    ...
                    let both_active = Bool::and(&[&passive_var.not(), &other_passive_var.not()]);
            ...
            let max_attacks = BV::from_u64(attacker_stats.num_attacks as u64, BV_HP_SIZE);
            let total_attacks: BV = defenders
                .iter()
                .map(|defender| {
            ...
            if attacker_stats.lumbering {
                ...
                let current_pos = attacker.as_z3();
                let moved = attack_hex_var.eq(&current_pos).not();
    ...
    pub fn create_dnf_constraint(
        ...
        let conjunct_vars = bool_vars_refs
            .iter()
            .map(|conjunct| Bool::and(conjunct))
            .collect::<Vec<_>>();

        let conjunct_vars_refs: Vec<_> = conjunct_vars.iter().collect();

        let disjunct = Bool::or(&conjunct_vars_refs);
...
pub fn generate_move_from_sat_model(
    model: &Model,
    graph: &CombatGraph,
    ...
    for (attacker, defenders) in &graph.attacker_to_defenders_map {
        ...
        let is_passive = model
            .eval::<Bool>(passive_var, true)
            .unwrap()
            .as_bool()
            .unwrap();
        ...
        let hex_val = model
            .eval::<Z3Loc>(attack_hex_var, true)
            .unwrap()
            .as_u64()
            .unwrap();
        let time_val = model
            .eval::<Z3Time>(attack_time_var, true)
            .unwrap()
            .as_u64()
            .unwrap();
        ...
        for defender in defenders {
            ...
            if let Some(num_attacks_var) = variables.num_attacks.get(&key) {
                let val = model
                    .eval::<Z3HP>(num_attacks_var, true)
                    .unwrap()
                    .as_u64()
                    .unwrap();
    ...
    for (friendly_loc, move_hex_var) in &variables.move_hex {
        ...
        let hex_val = model
            .eval::<Z3Loc>(move_hex_var, true)
            .unwrap()
            .as_u64()
            .unwrap();
        let time_val = model
            .eval::<Z3Time>(move_time_var, true)
            .unwrap()
            .as_u64()
            .unwrap();
...
fn loc_var_in_hexes(_ctx: &z3::Context, var: &Z3Loc, hexes: &[Loc]) -> Bool {
    if hexes.is_empty() {
    ...
    let alternatives = hexes
        .iter()
        .map(|hex| hex.as_z3().eq(var))
        .collect::<Vec<_>>();

    let alternatives_ref: Vec<_> = alternatives.iter().collect();

    Bool::or(&alternatives_ref)
}

fn bvsum(_ctx: &z3::Context, vars: &[BV]) -> BV {
    vars.iter()
        .fold(BV::from_u64(0, BV_HP_SIZE), |acc, var| acc.bvadd(var))
}
