    ...
    pub fn identify_combat_triples(&self, side_to_move: Side) -> Vec<CombatTriple> {
        ...
        for (attacker_pos, attacker) in &friendly_pieces {
            ...
            for (defender_pos, _defender) in &enemy_pieces {
                ...
                let attack_hexes = self
                    .get_valid_move_hexes(*attacker_pos)
                    .into_iter()
...
pub struct CombatGraph {
    ...
    // all friendly units
    pub friends: HashSet<Loc>,

    // all friendly units that can attack
    pub attackers: HashSet<Loc>,

    // all enemy units that can be attacked
    pub defenders: HashSet<Loc>,

    // all hexes that could be moved to by each friendly unit
    // if sufficient enemy units are removed
    pub move_hex_map: HashMap<Loc, HashMap<Loc, RemovalDNF>>,

    // all hexes given attacker can attack from
    pub attack_hex_map: HashMap<Loc, Vec<Loc>>,
...
impl CombatGraph {
    pub fn new(triples: Vec<CombatTriple>, friends: Vec<Loc>, board: &Board) -> Self {
        ...
        let mut attack_hex_map = HashMap::new();
        ...
        for friend in &friends {
            ...
            let free_move_hexes = board
                .get_unobstructed_move_hexes(*friend)
                .into_iter()
                .collect::<HashSet<Loc>>();

            let theoretical_move_hexes = board.get_theoretical_move_hexes(*friend);
            let dnfs = theoretical_move_hexes
                .iter()
                .map(|hex| {
